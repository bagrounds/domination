...
instance dynamicByteLengthRecord :: GEncodeArrayBuffer row list => DynamicByteLength (Record row) where
  byteLength xs = gPutArrayBuffer xs (Proxy :: Proxy list) >>= byteLength
...
instance encodeArrayBufferRecord ::
  ...
  putArrayBuffer b o xs = gPutArrayBuffer xs (Proxy :: Proxy list) >>= putArrayBuffer b o
...
instance decodeArrayBufferRecord ::
  ...
  readArrayBuffer b o = do
    ...
    case mObject of
      Nothing -> pure Nothing
      Just object -> Just <$> gReadArrayBuffer object (Proxy :: Proxy list)
...
class GEncodeArrayBuffer (row :: Row Type) (list :: RL.RowList Type) where
  gPutArrayBuffer :: Record row -> Proxy list -> Effect (O.Object ArrayBuffer)
...
instance gDecodeArrayBufferCons ::
  ...
  gReadArrayBuffer object _ = do
    ...
    rest <- gReadArrayBuffer object (Proxy :: Proxy tail)
