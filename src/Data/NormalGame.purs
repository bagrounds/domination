...
import Domination.Data.Table (Table(..), Table2(..), cardinality, smap, (&), (-#>), (-=>), (...), (<?<), (?), (?$), (?&?), (?@), (@?), (|=|), (|>), (|>|))
...
cleanup
  ...
  -> m NormalGame
cleanup
  game@{ longGame, phase, result, turn, card, card_class, card_pile
  , card_order, pile, pile_owner, pile_type, player, player_actions
  , player_bonus, player_buys, player_choice, choice, choice_class
  , choice_order } = f newDiscard
  where
    f = undefined "not implemented" -- ?f
    currentPlayer = turn
