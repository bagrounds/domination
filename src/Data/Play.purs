...
• Represents game actions, including starting a new game, playing a card, and making a purchase.
• Lens definitions for accessing nested fields within the `Play` type:
‣ `_playerIndex'`: Accesses the player index in various `Play` types.
‣ `_playerIndex`: Traverses and focuses the `_playerIndex'` lens.
• Automatic JSON encoding/decoding for `Play` type.
-}
