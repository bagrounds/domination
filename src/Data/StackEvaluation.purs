...
• Defines two data types: `StackExpression` and `StackValue`.
• Represents different operations that can be performed on a stack.
• Includes operations for choosing cards, duplicating, discarding, adding, drawing, trashing, and gaining to piles.
• Also includes conditional statements, bonus gain, and option handling.
• Data is encoded using Argonaut's JSON encoding mechanism.
-}
