...
• Purpose:
Define functions and data types for managing game stacks, including initialization, modification, and querying of stack contents.
• Functions:
• create a default supply with a specified player count.
• upgrade the entire supply by applying an upgrade function to each stack.
• find the index of a specific card in the supply.
• update a single stack at a specified index.
• retrieve a stack based on its index.
• calculate total positive and negative points across all stacks.
• identify the highest victory card stack (with optional checks for emptiness).
-}
