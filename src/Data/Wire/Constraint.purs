...
• Defines a data type for wire constraints.
• Represents four types of wire constraints:
• Up to a specific length (UpTo)
• Exactly at a specific length (Exactly)
• Down to a specific length (DownTo)
• Unlimited length (Unlimited)
• Provides functions and instances for decoding, encoding, showing, and encoding as JSON.
• Includes functionality for working with arrays of wire constraints.
-}
