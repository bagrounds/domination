...
• Module summary:
• Defines a data type `WireGame` for representing game state.
• Provides functions and instances for decoding, encoding, and manipulating the data.
• Purpose:
• Enables data conversion between `WireGame` and `Game`.
• Supports serialization and deserialization of game state.
-}
