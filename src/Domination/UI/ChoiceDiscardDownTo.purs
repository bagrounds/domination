...
data DiscardAction = ToggleDiscard Int Int | Done Int
  ...
  eval = H.mkEval H.defaultEval
    { handleAction = case _ of
      ToggleDiscard n i -> do
        ...
        H.modify_ $ mapWithIndex \j (Tuple c b) -> Tuple c (if i == j && (b || nKept > n) then not b else b)
      Done n -> (toResolved n <$> H.get) >>= H.raise
    }
  toResolved :: Int -> DiscardState -> Choice.Choice
  toResolved n xs = Choice.DiscardDownTo n (Just $ map snd $ filter fst (mapWithIndex (\i (Tuple _ b) -> (Tuple b i)) xs))

renderCardToDiscard :: forall a. Int -> Int -> Tuple Card Boolean -> HTML a DiscardAction
renderCardToDiscard n cardIndex (Tuple card selected) = HH.div
  ...
  [ HH.div (if Card.isVictory card then [ HP.class_ Css.victoryCard ] else [ HP.class_ Css.noVictoryCard ])
    [ HH.div (if Card.isAction card then [ HP.class_ Css.actionCard ] else [ HP.class_ Css.noActionCard ])
      [ HH.button
        [ HE.onClick \_ -> Just (ToggleDiscard n cardIndex)
        , HP.classes
          [ Css.card
          , if selected
            then Css.toTrash
