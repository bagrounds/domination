...
• Defines data types and functions for game rules.
• Provides predicates and checks for rule satisfaction.
Key components:
...
• ` Predicate` type: A function that takes an input and returns a boolean value.
• `Ruled` type: A structure containing the result of checking a rule.
Types:
• `Logic`: A type class representing logical values with various operations.
• `claim`: A constructor for representing a string claim.
• `Rule` and `Ruled` types.
Functions:
• `check`: Checks if a rule is satisfied, throwing an error otherwise.
• `rule`, `enforce`, `simpleRule`, `constantRule`, `appendErrorOn`, `appendError`, `lengthIs`.
-}
