...
* Provides a UI component for card selection in the Domination game.
* Allows players to choose cards from different piles based on constraints (e.g. exactly N, up to N).
* Renders a list of cards with toggle functionality and tracks the selection state.
* Supports visual feedback for selected/unselected cards and emits indices of selected cards when "Done" is clicked.
-}
